...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("goalDetail.button.showProperties", { defaultValue: "Show properties" })}
    >
...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goalDetail.breadcrumb.goals", { defaultValue: "Goals" }), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalDetail.breadcrumb.goal", { defaultValue: "Goal" }) }
    ]);
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <InlineEditor
          ...
          placeholder={t("goalDetail.placeholder.description", { defaultValue: "Add a description..." })}
          multiline
      ...
      <Tabs defaultValue="summary">
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("goalDetail.button.addSubGoal", { defaultValue: "하위 목표 추가 (Sub Goal)" })}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.subGoals", { defaultValue: "하위 목표가 없습니다. (No sub-goals.)" })}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goalDetail.empty.linkedProjects", { defaultValue: "연결된 프로젝트가 없습니다. (No linked projects.)" })}</p>
          ) : (
